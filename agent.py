...
class Agent:
    def __init__(self, env_name, n_iter, n_states, action_bounds, n_actions,bins,binspace, lr,heads,entropy_reg,system_merge):
        ...
        self.system_merge=system_merge
        self.current_policy = Actor(n_states=self.n_states,
                                    n_actions=self.n_actions,bins=bins,heads=heads).to(self.device)
        ...
        # self.scheduler = lambda step: float((step*0.)+ 1)
    ...
    def load_weights(self):
        checkpoint = torch.load("C:\\Users\gWX1180901\Desktop\mujoco\\"+self.env_name + "_weights.pth")
    ...
    def set_not_trainable(self,):
        for name,layer in self.current_policy.named_children():
            if "head" in name:
                for parameter in layer.parameters():
                    parameter.requires_grad = False

    def set_trainable_head(self,head):
        for name,layer in self.current_policy.named_children():
            if head == name:
                for parameter in layer.parameters():
